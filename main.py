...
##CONNECT TO DB
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get("HEROKU_POSTGRESQL","sqlite:///blog.db")
...
class BlogPost(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    # delete previous author column
    # gets author id from users table
    author_id = db.Column(db.Integer, db.ForeignKey("users.id"))
    # relationship to User class, back populates post relationship
...
def login():
    ...
    if request.method == "POST":
        ...
        user = User.query.filter_by(email=email).first()
        #check the stored hashed password against the entered password
        if not user:
            flash("An account does not exist for this email. Try again.")
            return redirect(url_for('login'))
...
#CHALLENGE - Add Delete Route for Comments User or Admin
